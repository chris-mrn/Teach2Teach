...
net_fm = Unet()
model_FM = GaussFlowMatching_OT(net_fm, device='cpu')
optimizer_fm = torch.optim.Adam(net_fm.parameters(), 1e-2)
model_FM.train(optimizer_fm, dataloader1 , dataloader0 , n_epochs=1000)
gen_FM_samples, hist_FM = model_FM.sample_from(X0[:10])

net_score = Unet()
model_score = NCSN(net_score, L=10, device='cpu')
optimizer_score = torch.optim.Adam(net_score.parameters(), 1e-2)
model_score.train(optimizer_score, epochs=1000, dataloader=dataloader1, print_interval=10)
